...
namespace motor_driver
{
    struct motorState
    {
        ...
        float velocity;
        float current;
        int temperature; // -40 - 215  (it seesm SERVO mode has a different range)
        int error_code;  // 0 means no fault, 1 means over temperature fault,
        // 2 means over current fault, 3 means over voltage fault, 4 means under voltage fault, 5 means
        // encoder fault, 6 means phase current unbalance fault (The hardware may be damaged)
        double timeNow; // computer time
    };
    ...
    {
    public:
        MotorDriver(const std::vector<int> &motor_ids,
        ...
        unsigned int motorReplyWaitTime = 1;  // micro seconds
    ...
    namespace default_params
    {
        ...
        // Working parameters for AK60-6 V2.2 firmware
        const motorParams AK60_6_V2p2_params = {
            -12.5, // P_MIN
            12.5,  // P_MAX
            -45,   // V_MIN
            45,    // V_MAX
