...
namespace motor_driver
{
      ...
      const std::vector<int> &enable_motor_ids)
  {
    ...
    // and then enable the motor. //Xiaobin: I don't think so....
      ...
      const unsigned char *CAN_reply_msg) const
  {
    ...
    auto timeSince = std::chrono::duration_cast<std::chrono::microseconds>(timeCurrent - timeStart);

    motorState state{.motor_id = id,
                     .position = p * current_params_.AXIS_DIRECTION,
                     .velocity = v * current_params_.AXIS_DIRECTION,
                     .current = i * current_params_.AXIS_DIRECTION,
                     .temperature = temp,
                     .error_code = error,
                     .timeNow = (double)timeSince.count()};
